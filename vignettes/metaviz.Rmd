---
title: "Visualize meta-analytic data with *R* package **metaviz**"
...
output: 
    rmarkdown::html_vignette:
      ...
      depth: 3
vignette: >
  %\VignetteIndexEntry{Visualize meta-analytic data with package metaviz}
...
In the following, we use four different example datasets distributed with the package **metaviz**. The datasets `mozart` and `homeopath` include a published meta-analyses using the Cohen d metric (Pietschnig, Voracek, & Formann, 2010; Mathie et al. 2017), `brainvol` contains data for a meta-analysis using correlation coefficients (Pietschnig et al., 2015), and `exrehab` contains data of a published meta-analysis with dichotomous outcomes and risk ratios as effect size (Anderson et al. 2016). More details can be found in the respective help files (`help(mozart)`, `help(homeopath)`, `help(brainvol)`, `help(exrehab)`).
...
For forest plots and thick forest plots it is possible to custimize the color of studies by suppplying a vector of colors to the `col` argument. The color of the summary effect(s) can be customized individually as well with the `summary_col` argument.
...
A novel variant of the funnel plot displays the power of studies to detect an effect of interest using a two-sided Wald test. The sunset (power-enhanced) funnel plot can be crated with the function `viz_sunset`. Per default the fixed meta-analytic summary is used as the underlying true effect for power computations. 

```{r, fig.asp = 0.75, fig.width = 8}
viz_sunset(homeopath[, c("d", "se")])
```

Many options for `viz_sunset` are provided. For instance, continuous power contours can be drawn, a user-specified underlying true effect chosen, and the significance level for power calication altered. See `help(brainvol)` for further details.
